...
local function getBody(char)
	return char:FindFirstChild(LockPart)
end
...
--== MOBILE SCOPE AIMLOCK ==--
local lastFOV = Camera.FieldOfView
RunService.RenderStepped:Connect(function()
	if isMobile then
		local currentFOV = Camera.FieldOfView
		AimEnabled = currentFOV < lastFOV
		lastFOV = currentFOV
	end
end)
...
local function removeESP(player)
	if ESPs[player] then
		...
		if ESPs[player].Name then ESPs[player].Name:Destroy() end
		ESPs[player] = nil
	end
end
